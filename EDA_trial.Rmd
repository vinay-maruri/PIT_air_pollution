...
#now the correct model is 99 lags.


```{r}
# Fit the VAR model (lag choices are 27, 99)
var_1 <- VAR(pollutants, p = 27, exogen = environmentalvariables)
...
fut_ev <- matrix(0, nrow = 2400, ncol = ncol(environmentalvariables)+1)
fut_ev[, 1] <- 1
for(i in 2:9){
  ...
  colnames(tmp) <- c("y", "ds")
  p <- prophet(tmp, weekly.seasonality = T)
...
fut_ev[, 10] <- sample(environmentalvariables$inversion, 2400, replace = TRUE)

fut_ev <- as.data.frame(fut_ev)

colnames(fut_ev) <- c("const", colnames(environmentalvariables))
```
...
```{r}
forecast_1 <- forecast(var_1, dumvar = fut_ev)
```
