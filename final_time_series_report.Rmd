...
[[PLACEHOLDER FOR INITIAL DISCUSSION OF RESULTS]]

When plotting a time series of the VARX model residuals, we see that the values are scattered somewhat randomly around a constant mean of zero. Moreover, these residuals resemble white noise. The only thing that we should be concerned about is the high occasional spikes but this could potentially be written off as a product of the large variation of data in the model.
...
While we feel confident in our analysis process and the results it produced, there were some fundamental limitations to some of the processes we employed in this project. 

The biggest constraint to performing the analysis was a lack of available data, because though we were able to receive the hourly weather data from 2016 to 2024 for multiple weather and pollutant parameters, much of this data was flawed or incomplete to the point that much of it could not be used. For example, many of the pollutant parameter values were missing during times when the other pollutant values were specified or certain parameters were reported as having a negative quantity of the pollutant appearing during certain time periods, which was an obvious impossibility and showed that there was a likely error in the data collection process for a section of our data. These unusable values of certain parameters meant that certain data simply had to be excluded, which limited the amount of data we had to work with and could have adversely impacted the overall accuracy of our analysis. 

Another constraint to our analysis process was the location limitation of our data, because though our data was an aggregation of weather conditions and pollutant quantities collected at multiple sites (Liberty, North Braddock, Lawrenceville, Parkway East, South Fayette, Avalon, USS Clairton Sodar Site), the number of pollutants being tracked at each site was not consistent. For example, while the Lawrenceville site that we centered our analysis on contained all the pollutant parameters of SO2, CO, Ozone, NO, PM10, and PM2.5, the Liberty site only contained the pollutant data for the SO2 parameter, and the Avalon site contained only the SO2 and PM2.5 pollutants, etc. Even though the ideal method to test the validity of the model would have involved checking the inversion occurrence and pollutant behavior at all sites, the lack of pollutant data at certain sites made this testing impossible, limiting our ability to understand how atmospheric inversion affects the pollutants at all sites. If this project were to be repeated or expanded upon in the future, we would definitely want to collect data ourselves to ensure that we have enough usable data from all sites to enhance the viability and accuracy of our analysis.

A final method of improvement would be reinforcing our method of inversion detection, as the approach we employed in this project was a naive approach that only used temperature and atmospheric data in regard to altitude to detect the occurrence of a cold air pocket being trapped under a hot air layer. While this approach should be valid, there may be a better approach to the detection process, though we could not find such reference material for building such a detection model or locate a data repository that contained data to locate an inversion occurrence. We only used the naive detection approach that also does not consider cases where changing winds might be caused by an environmental factor not linked to inversions, making the inversion detection process an important aspect of the project that should be improved in future iterations of the project. 

Operational limitations and possible methods of improvements aside, our results do indicate that our project objective of identifying and quantifying the effect that inversion events have on pollutants in the Pittsburgh Lawrenceville area was successful. These results showed a statistically significant increase in CO and NO pollutants, a statistically significant decrease in Ozone and PM10, and no statistically significant effect on SO2 or PM2.5, giving us some insight about how the quantity of those variables change upon the occurrence of an inversion. One possible utilization of this model and the resulting output in a real-world scenario could be using the result of variable significance to determine what kind of aid to provide an area with increasing pollutant quantities. For example, if the interested party was concerned about health risks or air quality, the results of of our analysis show that the significant increase in CO and NO pollutant levels might necessitate providing some kind of tangible aid to the target area such as masks, carbon monoxide detectors, air filtration systems, etc. Being able to preemptively provide material support to increase the quality of life for residents of a certain area is an invaluable benefit to public health, and such an implementation would be a logical extension of our current work. The utility of our work could also potentially be extended to other sites in Pittsburgh and beyond, so others can use our model and inversion detection method to quantify the effect of inversion on certain pollutants in the area.   
